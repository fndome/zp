...
├── gc_config.go       # GC 独占 CPU 配置 (GOMAXPROCS-1 + MemoryLimit)
├── main.go            # Go: OrderRequest / ProfitResult + zp.Batcher
├── src/
...
**工程上：CGO + `zp.Batcher` 攒批已经足够。** 100 次 CGO 调用的 10μs 开销，摊到 100 条请求上就是 100ns/条。与其冒兼容性风险搞 `.syso`，不如把攒批写好。
